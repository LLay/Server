 ...
 *          0 otherwise
 */
int process_arg(char *command, Calendar **calendar_list_ptr, User **user_list_ptr) {
  ...
  if (strcmp(command, "quit") == 0) {
    ...
    } else if (strcmp(command, "list_events") == 0 && cmd_argc == 2) {
        buf = list_events(calendar_list, cmd_argv[1]);
        // The next print statement will eventually be replace by writing to the client.
        printf("%s",buf);

    } else if (strcmp(command, "add_user") == 0 && cmd_argc == 2) {
        if (add_user(user_list_ptr, cmd_argv[1]) == -1) {
                error("User by this name already exists");
        }

    } else if (strcmp(command, "list_users") == 0 && cmd_argc == 1) {
        buf = list_users(user_list);
        // The next print statement will eventually be replace by writing to the client.
        printf("%s",buf);

    } else if (strcmp(command, "subscribe") == 0 && cmd_argc == 3) {
